 ...
 * Avoid adding jwt-decode dependency for simple payload extraction
 */
...
/**
 * Get user information from JWT token in localStorage
 */
const getUserFromToken = (): { name: string; email: string } | null => {
    try {
        ...
        if (decoded.exp && decoded.exp < now) {
            ...
            // Cleanup expired token to force re-login
 ...
 * Create personalized avatar when no profile picture available
 */
...
const DashboardLayout: React.FC = () => {
    ...
    const handleLogout = (): void => {
        ...
        // Prevent user from navigating back to protected routes
        navigate('/login', { replace: true });
    };
        ...
        <div className={styles.dashboardLayout}>
            ...
            >
                ...
                <nav className={styles.navGroup} aria-label="Điều hướng chính">
                    ...
                    {/* User Profile in Nav for Mobile/Tablet */}
                    {(isMobile || isTablet) && (
                        <div className={styles.navUserProfileContainer} ref={mobileMenuRef}>
                            <div
                                className={`${styles.navItem} ${styles.navUserProfile}`}
                                onClick={handleUserProfileClick}
                                onKeyDown={(e) => handleKeyPress(e, handleUserProfileClick)}
                                role="button"
                                tabIndex={0}
                                aria-label={`Menu người dùng: ${userName}`}
                ...
                <div className={styles.sidebarFooter}>
                    ...
                    <div className={styles.userProfileContainer} ref={dropdownRef}>
                        <div
                            ...
                            role="button"
                            tabIndex={0}
                            aria-label={`Thông tin người dùng: ${userName}`}
